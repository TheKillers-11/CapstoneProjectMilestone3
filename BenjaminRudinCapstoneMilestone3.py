...
def generate_graph(filtered_df,button_id=None,empty_df=None):
    # If an empty_df is passed, create an empty fig (designed for initial page load)
    if len(filtered_df)==0 and len(empty_df)>0:
        filtered_df = pd.DataFrame(empty_df)
        
    # If there is no title passed, use the default title
...
# Object that contains the wallet_graph on the page
...
def show_projection_graph(input_year,wallet_addresses):
    ...
    if input_year.isnumeric() and len(wallet_addresses)>0:
        # Call linear regression model here and generate graph based on generated dataframe
...
# Some random addresses I found online: all 3 will work and have a small to moderate number of transactions associated with them
# test values: 14bwkr3m8BWH8sgSXUiLVVS7CVEyHwz8sb, bc1q02mrh85muzdjk32sxu82022uke9qgjna6ydv05, 34xp4vRoCGJym3xR7yCVPFHoCNxv4Twseo
